...
- `dashboard-actions.test.ts` — misc `src/app/dashboard/actions.ts` Server Actions covering program/card RPC calls, plus `redeemAction`'s Telegram reward-redemption alert (sends when the vendor has a linked `chat_id`, skips silently when unlinked, and never changes `redeemAction`'s own success result when the lookup or the send itself fails) and its sibling `notifyCustomerByPhone` call (called with the card's phone and a redemption message, and a rejection there also never changes `redeemAction`'s own success result)
