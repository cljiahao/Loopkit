...
- `cards.test.ts` — `listCards`: fetches a vendor's cards for one program, optional phone search, mocks `createServerClient`; `activeCardCountsByProgram`: counts cards touched in the last 30 days per program (empty result for no programs, tallies by `program_id`), backing the Counter's busiest-program default; `programCardCount`: exact-count query scoped to `program_id`, null count coerced to 0, throws on error
...
- `program-access.test.ts` — `currentProgram`/`canCreateProgram`/`getEntitlement`: free/Pro program-count gating, and `getEntitlement`'s full shape including `allowedFamilies`/`allowedStampStyles`/`customColor`/`showBranding`; its `Program` fixture includes `birthday_bonus_enabled` (migration `0041`)
...
- `program.test.ts` — `programInputSchema`/`canPrepProgram`/`getEntitlement`: program validation and tier caps; `canUseFamily`/`canUseStampStyle`/`canUseColor`: free tier allows only the stamp family, `dots` style, and the default color (or unset), Pro allows every family/style/color; also covers `saveProgramSchema`/`buildProgramFields`'s stamp branch for `stamp_mark_mode`/`stamp_mark_preset` and the newer `stamp_style`/`stamp_color` (5 `StampVisualStyle`s, a `#rrggbb`-only hex), each carried into `config` unchanged or left `undefined` when blank; plus the Points Club `redemption_mode`/`catalog`/`offset_rate_points`/`offset_rate_dollars` fields — catalog min-length rejection, per-item fresh ids, and `stampsRequired`'s catalog/offset/neither fallback; plus the stamp variant's optional `reward_cost_dollars` (coerced, blank -> undefined, negatives rejected)
