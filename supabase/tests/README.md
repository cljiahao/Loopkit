...
- `rls.test.sql` — scoped to the highest-risk vendor-facing write paths
  (loopkit has 38+ migrations; exhaustive per-table coverage is out of
...
design.md` §3): `loopkit.vendors` (shared profile, for-all self policy),
  ...
  35 assertions, one rolled-back transaction, fixed-UUID inline fixtures —
  asserts RLS is actually enabled on every covered table, cross-vendor
  read/write isolation, that a grant gap (no `UPDATE` grant on
  `upgrade_requests`) fails on the table-level privilege check before RLS
  ever runs, and (for `provision_default_program`) that the
