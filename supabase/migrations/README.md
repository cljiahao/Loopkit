...
- `0042_loopkit_adjust_stamp.sql` — day-2 ops gap fix: `loopkit.adjust_stamp(p_program, p_phone, p_delta, p_reason)`, a reasoned `±` correction for classic Stamp-type cards, `owns_program`-gated like `add_stamp`, clamped at 0, required nonzero delta + non-blank reason, never creates a card (only UPDATEs an existing one). Widens `stamp_events_kind_check` to add `'adjust'`; logs `{delta, reason}` in the event's existing `payload jsonb` column rather than adding dedicated columns. Growth/Points/Chance cards (the jsonb `state` engine, 0004) are out of scope — the only correction tool for those is still Regenerate card.
