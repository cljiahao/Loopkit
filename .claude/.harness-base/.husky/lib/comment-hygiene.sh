#!/usr/bin/env bash
# Comment hygiene nudge. Invoked by husky's pre-commit hook. Warn-only
# (never blocks): flags change-narration comments and oversized comment
# blocks in staged files. Patterns come from .claude/comment-hygiene-patterns.txt.
