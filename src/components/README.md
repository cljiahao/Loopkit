...
- `scratch-card.dom.test.tsx` — jsdom tests: renders the cover/prize text, the scratch reveal path is fully undrawn (`stroke-dashoffset:100`) by default, draws in (`stroke-dashoffset:0`) while `scratching`, the whole overlay unmounts once `revealed` (with or without `scratching`), a `.scratch-reveal-shine` sweep once `revealed` and not before
- `scratch-card.tsx` — `ScratchCard`: two-layer card (reward/label content beneath a cover) for scratch-variant programs; the cover + "Scratch to reveal" text are masked by an irregular, randomized-once "scratched by hand" SVG path (`pathLength`-based `stroke-dasharray`/`dashoffset`, not a canvas — jsdom has no real canvas 2D context, and this repo's other reward-mechanic visuals are already SVG) that draws in while `scratching`, punching real transparent holes revealing the prize beneath rather than uniformly fading the cover's opacity; plays a one-shot `.scratch-reveal-shine` light sweep across the prize once `revealed` flips true; both disabled under reduced-motion
...
- `ui/`
- `wheel.tsx` — `Wheel`: SVG spinning-wheel visualization computing per-segment pie slices and a rotation transform that lands on `landedId`, reward segments styled gold; drives rotation via one continuous, monotonically-increasing React-state value + `transition-transform` for both the "still spinning" and final-settle phases (never a CSS `animate-spin` keyframe handed off to a separate transition — that handoff is a documented source of a visible jump, which is what read as "choppy"); settles via a strong `cubic-bezier(0.16,1,0.3,1)` ease-out (real friction-based deceleration, no overshoot/bounce)
