 ...
 * driver.js overlay. Auto-runs once on the shop-QR block for vendors who
 * haven't seen it (server-tracked via `seen`); the button replays it anytime.
 */
export function DashboardTour({ seen }: { seen: boolean }) {
  ...
  function onReplay() {
    if (pathname !== "/dashboard") {
      // Step 1 spotlights the shop QR block, so get there first, then run.
