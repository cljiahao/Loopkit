...
- `footer.tsx` — `Footer`: single-row site footer, matching qkit's exactly. Since 2026-09-16 a thin wrapper around `@merqo/ui`'s own `Footer` (`wordmark`/`tagline`/`kitName` slots) — loopkit's copy was found structurally identical to qkit's and paykit's, differing only in that content, and promoted. No bottom call-to-action band above it (removed to match qkit, which never had one).
- `footer.test.tsx` — asserts the wordmark link, tagline, copyright line, the About link, Terms/Privacy links, and sign-in link all render.
...
- `nav.tsx` — `Nav`: delegates the sticky-header shell to `@merqo/ui`'s `LandingNav` (`wordmark`/`end` slot component — sticky, bordered, `px-5 py-4 backdrop-blur-md`, `max-w-6xl` inner row, matching qkit's landing-header sizing exactly), supplying `Wordmark` + sr-only "loopkit home" text as `wordmark` and an `/about` link, an FAQ button (`/#faq`, hidden below `sm` — the leading `/` so it resolves correctly from `/about` too, not just from `/` where `Nav` also renders), plus either a Dashboard button (authed) or Sign in link + Get started button (signed out) as `end`
- `nav.test.tsx` — asserts the sticky header carries qkit's `px-5 py-4 backdrop-blur-md` classes, and that the About link points at `/about`
