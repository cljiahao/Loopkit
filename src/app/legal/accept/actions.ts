...
/**
 * Best-effort client IP from the standard forwarding headers. NOT trusted
 * (client-spoofable behind a permissive proxy) — this is an audit field on
 * the acceptance record, not an authz signal.
 */
 ...
 * owner of the acceptance record — loopkit has no local table for it) via
  ...
  for (const docType of DOC_TYPES) {
    const res = await fetch(`${merqoBaseUrl()}/api/merqo/legal-accept`, {
      ...
      body: JSON.stringify({
        ...
        doc_sha256: sha256(getLegalDocSource(docType, "loopkit")),
        kit_slug: "loopkit",
        ip,
