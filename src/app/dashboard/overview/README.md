...
- `base-strip.tsx` — `BaseStrip({ stats, redemptionRatePct, returnRatePct })`, client: the 4 base stats as `@merqo/ui` `StatTile`s wrapped in `@merqo/ui`'s `ElevatedCard`, each with an `InfoTooltip trigger="tap"` whose content is the stat's title/body/hint; below them the redemption-rate and return-rate figures, each with their own tap tooltip. `--` (two hyphens) for a null return rate
...
- `reward-cost-panel.tsx` — `RewardCostPanel({ cost, manageHref })`: rewards redeemed this month with `formatSgd(knownCostCents)` (`+` and a "programs have no cost set" note when a program's cost is unset), the pending-return-soon count, expired-unclaimed breakage, and a `<details>` per-program breakdown linking to `/setup`, wrapped in `@merqo/ui`'s `ElevatedCard`
