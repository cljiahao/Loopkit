...
- `counter/`
- `customers/`
- `dashboard-nav.dom.test.tsx` — jsdom tests asserting `DashboardNav`'s inline nav links, mobile menu toggle, `data-tour` anchors, account-dropdown item order (Profile/Settings/Plan/Get help/Feedback/Sign out), that the trigger and the opened dropdown both show the real stall name (never a static "Vendor account" label or the vendor's email, falling back to "Your stall" when unset), the tier badge, and that submitting Feedback/Get help calls `submitFeedbackAction`/`submitSupportMessageAction` with the right shape (including the `message`→`body` field mapping) and surfaces a failed submit inline
- `dashboard-nav.tsx` — client `DashboardNav`: composes `@merqo/ui`'s `DashboardNav` (wordmark, Dashboard/Customers/Activity/Stats nav links, mobile burger, `data-tour` anchors) with its internal `AccountMenu` (avatar trigger, Profile/Settings/Plan/Get help/Feedback/Sign out dropdown, owning its own Feedback/Get-help Sheet chrome). `vendor.subtitle` carries the stall name — `@merqo/ui`'s `AccountMenu` has one subtitle line shared by the trigger and the dropdown header, so unlike the old hand-rolled version there's no separate "Vendor account" descriptor; falls back to "Your stall" when unset. `getHelp`'s `onSubmit` and `onFeedbackSubmit` are throw-adapters around `submitSupportMessageAction`/`submitFeedbackAction`, since those return a `{success,error}` result object but `@merqo/ui`'s contract expects a rejected promise on failure; `isSupportCategory` narrows the sheet's optional `category` string against `SUPPORT_CATEGORY_LABELS`' real keys before it reaches the action, falling back to `"other"`
