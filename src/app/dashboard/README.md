...
- `layout.tsx` — async `DashboardLayout` server component: requires vendor auth, redirects admins to `/admin`, renders the sticky header (`px-5 py-3.5`, matching qkit's dashboard-header sizing exactly) with `DashboardNav`, defines an inline sign-out server action, and reads `vendors.tour_seen_at` to render `@/components/dashboard-tour`'s `DashboardTour` (own README), passing `seen={!!vendorRow?.tour_seen_at}`
