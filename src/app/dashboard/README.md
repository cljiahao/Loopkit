...
- `counter/`
- `customers/`
- `dashboard-nav.dom.test.tsx` — jsdom tests asserting `DashboardNav`'s inline nav links, mobile menu toggle, and account-dropdown item order (Profile/Settings/Plan/Get help/Feedback/Sign out, no duplicate Customers item), that Feedback opens the `FeedbackForm` Sheet, that Get help is a `mailto:` link, that the trigger button itself shows the stall name beside the avatar (or an "Account" fallback), and that the opened dropdown's label shows the stall name (or a "Your stall" fallback) with a static "Vendor account" subtitle, never the vendor's email
- `dashboard-nav.tsx` — client `DashboardNav`: sticky header with a left-hand group (mobile burger — left of the brand, opposite the account menu — brand, Dashboard/Customers/Activity/Stats nav links) and a right-hand account trigger (avatar + `vendorName || "Account"` + a chevron, `md:` and up) opening a dropdown (Profile/Settings/Plan/Get help/Feedback/Sign out) whose label shows the stall name (`vendorName || "Your stall"`) and a static "Vendor account" subtitle — matching qkit's trigger and dropdown exactly, never the vendor's email; the trigger previously showed only the bare avatar, with the stall name visible only once the dropdown was opened — a parity gap qkit didn't have; the mobile burger and the inline Dashboard/Customers/Activity/Stats links are shadcn `Button`s (`variant="ghost"`) rather than hand-rolled `<button>`/`<Link>` markup, matching qkit's pattern, and the burger now carries `aria-expanded={mobileOpen}` alongside its `aria-label`; Get help is a `mailto:` link, Feedback opens a `Sheet` rendering `FeedbackForm`; the mobile link panel closes on a tap-away scrim as well as the burger toggle; the burger, account-menu trigger, and each desktop nav link carry `data-tour` anchors (`nav-menu`, `nav-account`, `nav-dashboard`/`nav-customers`/`nav-activity`/`nav-stats`) for the onboarding tour
