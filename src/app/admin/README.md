...
- `page.tsx` — `AdminOverviewPage`: platform-wide totals (programs, customers, stamps, rewards), a recent cross-shop activity feed, and the live-pricing admin form (`PricingFormClient`), wrapped in `@merqo/ui`'s `ElevatedCard`.
...
- `programs/`
- `stat.tsx` — `Stat`: a small labeled-value tile (`@merqo/ui`'s `ElevatedCard`-based) used across the admin overview and program detail pages.
- `vendors/`
