...
- `page.tsx` — `AdminVendorsPage`: fetches `listVendors()` and `listPendingUpgradeRequests()`, renders the pending-requests section and a vendor table (both `ElevatedCard`-wrapped, the table via `@merqo/ui`'s shared `DataTable`) with Pro badges/toggles.
