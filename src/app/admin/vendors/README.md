...
- `page.tsx` — `AdminVendorsPage`: fetches `listVendors()` and `listPendingUpgradeRequests()`, renders the pending-requests section and (via `VendorsTable`) the vendor table, both wrapped in `@merqo/ui`'s `ElevatedCard`.
