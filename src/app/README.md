...
the legal-acceptance gate inside `requireVendor`
(`src/features/auth/api/require-vendor.ts`), loopkit's single vendor-gate
entry point. `api/` exposes route handlers consumed by the merqo parent
app over HTTP. The root-level files (`layout.tsx`, `globals.css`,
