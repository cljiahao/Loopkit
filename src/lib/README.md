...
- `cards.ts` — `listCards` (the signed-in vendor's cards for one program, optional phone search, most-recently-updated first) and `activeCardCountsByProgram` (cards touched in the last 30 days, counted per program, for the Counter's default-program pick)
