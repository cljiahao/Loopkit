...
- `cards.ts` — `listCards` (the signed-in vendor's cards for one program, optional phone search, most-recently-updated first), `activeCardCountsByProgram` (cards touched in the last 30 days, counted per program, for the Counter's default-program pick), and `programCardCount` (exact `count(*)` of one program's cards, one row per customer, backing the setup form's edit-impact confirmation)
- `customer-segments.ts` — pure filtering and ordering for the vendor-wide Customers list: `customerSegments`/`filterBySegment` (all / ready / new / lapsed as independent views), `sortCustomers` (recent / away / progress), `segmentCounts` (one-pass chip counts), and `parseSegment`/`parseSort` which validate an untrusted query value against a fixed allowlist and default otherwise
- `customers.ts` — `aggregateCustomers` (pure phone-keyed merge of customers+cards across programs; takes a `programsById` map of `{ name, stampsRequired }` and derives `firstSeenAt`, `rewardReady`, `bestGap`, and the most-recently-touched card's `recentProgramId`/`recentProgramName` per row), `listVendorCustomers` (impure shell, RLS-scoped, fetches `first_seen_at` and the cards' `updated_at` to feed those fields), and `getCustomerDetail(phone)` (one customer's name/last-seen plus every card they hold across the vendor's own programs, backs `/dashboard/customers/[phone]`)
- `engine/`
...
- `legal-gate.ts` — `checkLegalAcceptance(email)`: is this vendor's terms/privacy acceptance current with `@merqo/ui`'s `LEGAL_VERSIONS`? loopkit owns no acceptance record — merqo does — so the real check is a bearer-authed `GET ${MERQO_BASE_URL}/api/merqo/legal-status`, cached in `loopkit.legal_check_state` (migration `0044`) for a 5-minute TTL via the service-role client; fails closed (`false`) on a missing `MERQO_CUSTOMER_SECRET`, an unreachable merqo, a non-2xx response, or a stale/malformed accepted-version body. `requireCurrentLegalAcceptance(email)` is the page/layout guard companion: `redirect("/legal/accept")` when the check fails, a no-op when `email` is falsy (test: `legal-gate.test.ts`)
...
- `money.ts` — SGD money helpers: `dollarsToCents` (`Math.round(d * 100)`), `centsToDollars`, `formatSgd` (`"$1.20"`). One place for the dollars/cents rounding, since the DB stores integer cents (`programs.reward_cost_cents`) and forms collect dollars
- `phone.ts` — `normalizePhone`: validates an SG mobile number (starts 3/6/8/9, 8 digits) and returns E.164 `+65…`; `maskPhone`: vendor-facing partial mask for dashboard lists (keeps the first 4 local digits, masks the rest)
- `program-config.ts` — pure, server-import-free program config builders: `buildPlantConfig` (5-stage growth/decay config from a single visits-to-bloom knob, stage names "Empty/Sip/Half Full/Nearly Full/Full" for the cup variant — the 50%-threshold stage was previously mislabeled "Quarter Full") and `buildChanceConfig` (wheel/scratch segment config with fresh per-segment ids, threading each segment's optional vendor-picked `color` through, plus an optional 5th `scratchCoverStyle` argument — applied only when `variant === "scratch"` — for the vendor's chosen cover material); kept free of `next/headers` so client bundles (`preview-state.ts`) can import it directly; `segmentWinPercent`/`overallWinPercent` (pure, same weight math `chance.ts`'s `pickSegment` uses internally) surface a segment pool's actual win odds as percentages for the Basics segment editor; `segmentInputSchema` validates an optional `color` as a `#rrggbb` hex string; `pointsCatalogItemInputSchema`/`PointsCatalogItemInput` (Points Club's catalog-item shape, no `id` — assigned server-side like Wheel/Scratch segments). Re-exports `ScratchCoverStyle` (`"foil" | "wax" | "ticket"`, from `engine/chance.ts`), `StampVisualStyle` (`"dots" | "seal" | "ink" | "punch" | "charm"`, from `engine/stamp.ts`), and `PointsRedemptionMode`/`PointsOffsetRate` (from `engine/stamp.ts`) so client-safe consumers like `preview-state.ts` and `setup-form.tsx` don't need to reach into the engine layer directly
...
- `program.ts` — `Program`/`SaveProgramInput` types, `programInputSchema`/`saveProgramSchema` (discriminated-union Zod schema per program type — the stamp variant's `birthday_bonus_enabled` is the only per-type-plus-edit-only field, wired by `src/app/setup/actions.ts`'s `updateExistingProgram`, not `createNewProgram`'s `create_program` RPC; the scratch variant's own optional `scratch_cover_style` field, `z.enum(["foil", "wax", "ticket"])`, is scratch-only — absent from the wheel branch; the stamp variant's own optional `stamp_style`/`stamp_color` fields — `z.enum(["dots", "seal", "ink", "punch", "charm"])` and a `#rrggbb`-only hex — are free-tier-gated: only `dots` and the default color save without Pro, see `canUseStampStyle`/`canUseColor` below; the stamp variant's own optional `redemption_mode`/`catalog`/`offset_rate_points`/`offset_rate_dollars` fields, points-only; the stamp variant's own optional `reward_cost_dollars` field (a dollars input, coerced, `>= 0`) which `updateExistingProgram` converts to `programs.reward_cost_cents` on edit only — `Program` carries the read-back `reward_cost_cents`, also in `PROGRAM_COLUMNS`), `buildProgramFields` (passes `data.scratch_cover_style` through to `buildChanceConfig` for scratch programs, `undefined` for wheel; passes `data.stamp_style`/`data.stamp_color` straight into the stamp branch's `config`; assigns each catalog item a fresh id server-side and computes `stampsRequired`'s now-synthetic fallback — max catalog cost, or the offset rate's `points` — rather than trusting whatever the now-hidden form field submitted), `listPrograms`/`getProgramById`/`currentProgram`, `Entitlement`/`getEntitlement`/`canCreateProgram`/`canPrepProgram` (free vs. pro tier caps: program count, plus `allowedFamilies`/`allowedStampStyles`/`customColor`/`showBranding`) and `canUseFamily`/`canUseStampStyle`/`canUseColor` (pure checks against those fields, enforced in `src/app/setup/actions.ts`'s `mechanicGateError` on every create/edit/change-type/prep; free tier is stamp/dots/default-color/branded-poster only), `isPro`, `applyDueCutovers` (lazy scheduled-retirement cutover), `getProgram` (transitional single-program shim), `VoucherLookup`/`getVoucherByToken` (owner-gated read for the `/dashboard/redeem-voucher` scan-confirm screen, via the `voucher_by_token` RPC)
...
- `safe-redirect.ts` — `safeRedirectPath(next, fallback)`: pure open-redirect guard for a `next` query/form param, accepting only a same-origin relative path (leading `/`, not `//` or `/\`, no embedded control characters) and falling back otherwise; used by `/legal/accept`'s page and server action (test: `safe-redirect.test.ts`)
...
- `stats.ts` — `classifyActivity`/`pctChange`/`bucketVisitsByDay`/`avgDaysBetweenVisits`/`computeCardStats` (pure aggregation pipeline), plus the vendor-dashboard Overview helpers `returnRate90d` (fraction of 90-day-active cards that are repeat visitors, the "% come back" headline, `null` when no card is 90-day-active), `regularsGoneQuiet` (cards with 3+ lifetime visits whose last visit is 21 to 40 days ago, the win-back list), and `cardsNearReward` (count of cards one or two stamps short of the reward); `countRegulars` (cards with 2+ lifetime events AND one in the last 30 days, the spec's "Regular"), `countNewThisMonth` and `sgtMonthStart` (SGT calendar-month boundary) power the Overview base stats; plus `getProgramStats`/`getVendorStats` (impure shells fetching cards+stamp_events), `getVendorOverviewInputs` (a separate cards+events query returning the classified event arrays plus per-card `program_id`/`stamp_count`/`created_at` for the Overview, independent of `getVendorStats`) and `countExpiredVouchers` (impure shell counting `reward_vouchers` that expired in the last 30 days — a separately-sourced tile added alongside, not replacing, `rewards30d`/`redemptionRate` per `docs/superpowers/specs/2026-07-16-reward-voucher-ledger-design.md`) — powers the vendor stats dashboard. `mechanicLabel`/`computeMechanicBreakdown` (pure) plus `getVendorMechanicBreakdown` (impure shell) group the same cards/events by which named mechanic (Stamp/Growth/Chance Card) a program's engine `type` belongs to — "Points" is a `stamp.ts` render variant, not a distinct DB type, so it's grouped under Stamp rather than guessed at from config.
- `supabase/`
...
- `types.ts` — `Json` type, `SocialLinks` (shape of the shared `merqo.vendor_profile.social_links` JSONB column — not part of the `loopkit` schema), and the hand-written `Database["loopkit"]` interface (Row/Insert/Update per table, including `vendor_notify_settings` and `legal_check_state`), a manual mirror of `supabase/migrations/` kept in sync by hand (no live DB codegen yet)
...
`legal-gate.ts` is a third kind of merqo HTTP call (kit → merqo, like
`merqo-customer-notify.ts`, not the RPC-plumbing group above) — its
`requireCurrentLegalAcceptance` is called from `requireVendor`
(`src/features/auth/api/require-vendor.ts`), loopkit's single gate entry
point, and its `checkLegalAcceptance` is reused directly by
`src/app/legal/accept/actions.ts` to prime the cache on a fresh accept.
