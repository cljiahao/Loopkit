...
- `telegram-link.ts` — service-role impure shell backing the dashboard settings "Connect Telegram" section: `getOrCreateTelegramLinkToken(vendorId)` reuses an existing unexpired `telegram_link_tokens` row or mints a fresh 30-minute-expiry one (both tables have no client write grant, so every call runs on the service-role client); `disconnectTelegram(vendorId)` deletes the vendor's own `vendor_telegram` row (`test: telegram-link.test.ts`)
