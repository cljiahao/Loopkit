...
- `program.ts` — `Program`/`SaveProgramInput` types, `programInputSchema`/`saveProgramSchema` (discriminated-union Zod schema per program type — the stamp variant's `birthday_bonus_enabled` is the only per-type-plus-edit-only field, wired by `src/app/setup/actions.ts`'s `updateExistingProgram`, not `createNewProgram`'s `create_program` RPC; the scratch variant's own optional `scratch_cover_style` field, `z.enum(["foil", "wax", "ticket"])`, is scratch-only — absent from the wheel branch; the stamp variant's own optional `stamp_style`/`stamp_color` fields — `z.enum(["dots", "seal", "ink", "punch", "charm"])` and a `#rrggbb`-only hex — are free for every vendor, no Pro gate, matching every researched competitor's own free-tier customization), `buildProgramFields` (passes `data.scratch_cover_style` through to `buildChanceConfig` for scratch programs, `undefined` for wheel; passes `data.stamp_style`/`data.stamp_color` straight into the stamp branch's `config`), `listPrograms`/`getProgramById`/`currentProgram`, `Entitlement`/`getEntitlement`/`canCreateProgram`/`canPrepProgram` (free vs. pro tier caps — program count only; deliberately not extended for stamp style/color, see above), `isPro`, `applyDueCutovers` (lazy scheduled-retirement cutover), `getProgram` (transitional single-program shim)
