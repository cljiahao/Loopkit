...
- `stamp.ts` — `stampStrategy`: classic N-stamps-for-a-reward counter with `dots`/`flame`/`points` view variants, a configurable `points_per_visit` increment, and an optional `stamp_mark` config (`{mode, preset}`) surfaced on the `dots` view as `markMode`/`markPreset` for a vendor's chosen stamp icon/photo
