...
- `2026-09-08-vendor-dashboard-redesign-design.md` — "Vendor dashboard redesign": rebuilds Overview / Counter / Customers around the vendor's real questions (are regulars still coming, what is the reward liability costing me, who to act on today). Overview becomes a daily briefing of `stats.ts` metrics instead of a program launcher; Counter goes scan-first with an empty-until-loaded active card; Customers gets segment chips + sort. Adds `programs.reward_cost_cents` (one cost per program's single reward) and 3 pure `stats.ts` helpers. Phase 1 ships ungated. `reward_tiers` / products catalog / Pro gating / customer-facing view are later phases. Split into 5 sub-plans, all now drafted in `plans/2026-09-08-*`. Resolved 2026-09-08: earned vouchers default to a 90-day expiry, gating stays deferred to Phase 2, the shop join QR stays vendor-wide (program-specific `&p=` deferred with `vendor_join`'s all-programs semantics).
- `2026-09-09-program-edit-safety-design.md` — "Program Edit Safety + Reward Expiry Default": the two gaps left after the dashboard redesign. Part 1 adds a client-side `AlertDialog` before a progress-affecting edit (`stamps_required` or `reward_text` changed, compared on the built values) to a program that already has cards, spelling out what current cardholders see; non-progress edits and zero-card programs save silently as before (`src/lib/program-edit-impact.ts` pure helper, `src/app/setup/edit-impact-dialog.tsx`). Server actions unchanged. Part 2 lands the deferred 90-day reward-expiry default at the creation layer only: `create_program` gets `p_reward_expiry_days int default 90` (migration `0046`) and the setup form pre-fills 90 for new programs, with `null` still meaning "never expires" (now reached by clearing the field). No backfill, no `grant_reward_voucher` change. Archive / Replace explicitly out of scope (already shipped via `replaced_by` / `?migrate=`).
