...
- `2026-09-09-program-edit-safety-design.md` — "Program Edit Safety + Reward Expiry Default": the two gaps left after the dashboard redesign. Part 1 adds a client-side `AlertDialog` before a progress-affecting edit (`stamps_required`, `reward_text`, or a mechanic knob) to a live program with active cards, spelling out what current cardholders see; non-progress edits and zero-card programs save silently as before (`src/lib/program-edit-impact.ts` pure helper, `src/app/setup/edit-impact-dialog.tsx`). Part 2 lands the deferred 90-day reward-expiry default at the creation layer only: `create_program` gets `p_reward_expiry_days int default 90` (migration `0046`) and the setup form pre-fills 90 for new programs, with `null` still meaning "never expires" (now reached by clearing the field). No backfill, no `grant_reward_voucher` change. Archive / Replace explicitly out of scope (already shipped via `replaced_by` / `?migrate=`).
