...
2. **Apply the migrations** — SQL Editor, in order:
   ...
   - apply `0024_loopkit_head_start_percent.sql` — adds
     `programs.head_start_percent` (integer, NOT NULL default 20, CHECK
     5–50) so vendors set how much of a head start a new stamp/plant card
     gets, replacing the old flat ~20% seed. Recreates `create_program`
     (additive `p_head_start_percent`, defaulted) and `enroll_card`
     (stamp/plant seed now scales by the column). Plant's Sprout-stage
   ...
   - apply `0036_vendor_telegram.sql` then `0037_drop_vendor_telegram.sql`
     — `0036` added `loopkit.vendor_telegram`/`loopkit.telegram_link_tokens`
     backing loopkit's own (now-retired) Telegram reward-redemption alerts
     bot; `0037` drops both tables. Reward-redemption vendor alerts now go
     through merqo's own shared Telegram bot instead (`notifyVendor` in
     `src/lib/merqo-customer-notify.ts`, `POST /api/merqo/notify-vendor`)
     — no loopkit-side bot setup needed. A vendor who'd linked loopkit's
...
2. Environment Variables (Production + Preview) — **same shared Supabase project**:
   ...
   - `MERQO_BASE_URL` / `MERQO_CUSTOMER_SECRET` — optional; power both the
     customer-notify reuse call and the vendor Telegram alert (`notifyVendor`,
     `notifyCustomerByPhone` in `src/lib/merqo-customer-notify.ts`). Leave
     both unset to ship without either — they no-op rather than throw.
